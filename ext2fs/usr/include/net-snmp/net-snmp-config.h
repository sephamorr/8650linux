...
#define NETSNMP_CONFIGURE_OPTIONS " '--prefix=/usr' '--sysconfdir=/etc' '--sbindir=/usr/bin' '--mandir=/usr/share/man' '--enable-ucd-snmp-compatibility' '--enable-ipv6' '--with-python-modules' '--with-default-snmp-version=3' '--with-sys-contact=root@localhost' '--with-sys-location=Unknown' '--with-logfile=/var/log/snmpd.log' '--with-mib-modules=host misc/ipfwacc ucd-snmp/diskio tunnel ucd-snmp/dlmod' '--with-persistent-directory=/var/net-snmp' '--disable-static' 'CFLAGS=-march=armv5te -O2 -pipe -fstack-protector --param=ssp-buffer-size=4' 'LDFLAGS=' 'CPPFLAGS=-D_FORTIFY_SOURCE=2'"
